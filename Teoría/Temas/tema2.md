...
Un protocolo define una serie de tipos de servicios. Dentro de un mismo protocolo hay varios dos tipos de mensajes: mensaje de solicitud, mensaje de respuesta. El mensaje más gordo es el de respuesta. Las cabeceras tienen que tener una sintaxis, es decir, un formato.
...
**¿Que los definen?**: Lo que define a un protocolo es:
  ...
  - **El tipo de mensaje**:
    - Request: petición
    - Responde: respuesta
  - **La sintaxis**: Definición y estructura de sus campos en el mensaje.
...
Existen pues diferentes tipos de protocolos, la primera es la más importante:
...
La tendencia hoy en día, para no trabajar con cabeceras muy grandes, es hacer cabeceras flexibles, es decir, poner una cabecera fija muy pequeñita y luego añadir trozos (chunks) según lo que necesitemos. Al principio de cada trozo se indica su longitud y la información que contiene dicho trozo.

Los trozos pueden incluir una cabecera específica con una serie de datos en forma de parámetros:
  - **Una cabecera fija**.
  - **Parámetros de longitur variable**

Los parámetros usan un formato TLV, los primeros 16 bits los usamos para especificar el tipo de parámetro que se va a leer, los siguientes 16, para especificar el tamaño del parámetro y, por último, ya se lee el parámetro en cuestión.

Todo ello implica una serie de **características** para las aplicaciones de red que son:
...
Dependiendo si están orientados a conexión o no existen dos tipos principales de protocolos:
...
La **resolución** o búsqueda del objetivo se puede realizar de dos formas diferentes:
  ...
  - **Recursiva**: el host que realiza la consulta se la manda a un servidor. Si este no la puede resolver, se la realizará a otro, y este a otro si no la puede resolver, y así sucesivamente.
    ...
    Además se presenta el uso de caché: el host realiza la petición al servidor local, este mira en caché y si puede responder, responde y sino, le envía la consulta a otro servidor remoto.
...
Existen disferentes **tipos de mensajes** para HTTP:
  - **Request**, mensaje que puede ser del tipo:
  ...
  - **Response**: Nos dice la versión de http y un códifo acompañado de una pequeñas explicación.
    - **200**: Los datos se están sirviendo.
    - **300**: Código de error.
    - **400**: Error de página u objeto, no se encuentra objeto que se necesitaba.
...
### 4.1 Otros protocolos usados
  - **Protocolos de acceso** (POP3): El protocolo POP3, al igual que SMTP, tiene una fase de conexión, una de servicio y otra de cierre.
...
Existen pues, distintos mecanismos de seguridad:
